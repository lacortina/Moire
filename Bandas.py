...
#--------------FUNCIONES DE AYUDA A BUCLE PRINCIPAL ----------------------------------
...
for ik, kf in enumerate(kpoints_frac):
    k = kf[0]*Red_reciproca[0] + kf[1]*Red_reciproca[1] + kf[2]*Red_reciproca[2]
    """ k = 0.0000000000*Red_reciproca[0]
    for ik in range(18):
    k += Red_reciproca[0]*0.005
    print(k) """
    ...
    eigenvaluesL, eigenvectorsL = eigh(HL, SL) 
    ...
    for n in range(N):
        vec = eigenvectorsL[:, n]  # vector columna de dimensión N
...
#print(SacarterminoSK('Mo', 'S', 'Vpdp', SK), SacarterminoSK('Mo', 'S', 'Vpds', SK))
#print(SacarterminoSK('S', 'Mo', 'Vdpp', SK), SacarterminoSK('S', 'Mo', 'Vdps', SK))
